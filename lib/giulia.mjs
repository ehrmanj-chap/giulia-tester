...
export function createGiulia({ config, provider }) {
  return {
    async chat(rawMessages) {
      ...
      const knowledgeMeta = {
        cultural: { sha256: culturalKb.sha256, chars: culturalKb.chars, files: culturalKb.documents.map(d => d.file), chunks: culturalKb.index.count },
        business: { sha256: businessKb.sha256, chars: businessKb.chars, files: businessKb.documents.map(d => d.file), chunks: businessKb.index.count }
      };
